## Olá! Eu sou o **Vitor Hugo**.

Engenheiro Mecânico e Tecnólogo em Eletrônica Industrial.

Bem-vindo ao meu portfólio! Aqui você encontra os projetos que desenvolvi nas áreas de:

## 🚀 Áreas de Atuação

### ⚙️ Sistemas Embarcados & IoT
Desenvolvimento de firmware.
- **Hardware:** ESP32.
- **Linguagens:** C, C++.
- **Protocolos & Tecnologias:** 

### 📊 Engenharia de Dados
Construção de pipelines para transformar dados brutos em insights acionáveis.
- **Linguagens:** 
- **Ecossistema:** 

### 🤖 Inteligência Artificial
Aplicação de modelos preditivos.
- **Foco:** 
- **Frameworks:** 

---

## 🛠️ Tecnologias e Ferramentas

| Categoria | Ferramentas |
| :--- | :--- |
| **Linguagens** | C, C++, Python, SQL |
| **Firmware/IDE** |ESP-IDF, Arduino IDE, VS Code |
| **Data/AI** |  |

---


## 📫 Vamos nos conectar?

- 💼 [Vitor Hugo](www.linkedin.com/in/vitorhugo28)
- 📧 vitorh411@gmail.com
- 📍 São Paulo / Remoto







